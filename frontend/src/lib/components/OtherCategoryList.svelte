	...
	{:else}
		<div class="mb-3 p-3 bg-white rounded border border-gray-200">
			<div class="text-sm text-gray-700">
				...
				<strong class="text-gray-900">{(totalProbability * 100).toFixed(1)}%</strong> total probability
			</div>
			<p class="text-xs text-gray-600 mt-1">These tokens have probabilities below the threshold</p>
		</div>
		...
		{#if sampleTokens.length > 0}
			<div>
				...
				<div class="space-y-1 max-h-48 overflow-y-auto">
					{#each sampleTokens as token (token.token_id)}
